...
# AI investavimo aplikacija su lanksčiomis rekomendacijomis
...
with st.sidebar:
    selected = option_menu(
        ...
        options=["Apžvalga", "Sentimentas", "Rekomendacijos", "AI prognozė", "Lanksčios rekomendacijos"],
        icons=["graph-up", "chat-left-text", "stars", "cpu", "sliders"],
        menu_icon="cast",
        default_index=0,
    )
...
elif selected == "Lanksčios rekomendacijos":
    st.title("Lanksčios rekomendacijos pagal AI")
    min_return = st.slider("Minimali 60 d. grąža (%)", min_value=-10, max_value=20, value=0)
    min_sentiment = st.slider("Minimalus naujienų sentimentas", min_value=-1.0, max_value=1.0, value=0.0, step=0.05)

    recommendations = []
    for ticker in tickers:
        returns = calculate_returns(ticker)[0] if calculate_returns(ticker) else None
        sentiment_data = get_news_sentiment(ticker)
        if sentiment_data:
            sentiment_score = sum(s['sentiment'] for s in sentiment_data) / len(sentiment_data)
            if returns is not None and sentiment_score >= min_sentiment and returns >= min_return:
                recommendations.append((ticker, round(returns, 2), round(sentiment_score, 2)))

    if recommendations:
        st.success("Rekomenduojamos akcijos pagal pasirinktus kriterijus:")
        for r in recommendations:
            st.write(f"**{r[0]}** – Grąža: {r[1]}%, Sentimentas: {r[2]}")
    else:
        st.warning("Nerasta akcijų, atitinkančių pasirinktus kriterijus.")
