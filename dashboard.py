...
def forecast_stock_price(ticker):
    ...
    X = []
    y = []
    for i in range(50, len(scaled_data) - 7):
    ...
    dates = pd.date_range(df.index[-1] + timedelta(days=1), periods=7)
...
# --- AI prognozė ---
    ...
    with st.spinner("Mokome AI modelį..."):
        history, prediction = forecast_stock_price(ticker)
    if history is None or prediction is None:
        ...
        fig.update_layout(title=f"{ticker} prognozė (7 d.)", xaxis_title="Data", yaxis_title="Kaina (USD)")
        st.plotly_chart(fig, use_container_width=True)
